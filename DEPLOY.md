...
docker compose exec -T db sh -c 'exec mysql -uroot -p"$MYSQL_ROOT_PASSWORD" jira_monitor' < /root/jm-backup.sql
...
## 12) الهوية + الربط بجيرا من الواجهة

بعد السحب، **أعد تشغيل `npm run seed:admin`** مرة لإضافة الصلاحيات الجديدة
(`manage_branding` · `manage_integration` · `act_tickets`) لدور Admin.

- **الهوية** (Admin → الهوية): ارفع شعاراً وخلفية وأيقونة متصفح. تُخزَّن في `uploads/`
  (مُستبعَد من git) وتُخدَم عبر `/api/branding/asset/<type>`. الخلفية تملأ الشاشة.
- **الربط بجيرا** (Admin → الربط بجيرا): عدّل رابط جيرا/البريد/التوكن/JQL/المسار/حجم الصفحة
  لإعادة التوجيه لمشروع آخر، مع زر **اختبار الاتصال**. المصدر الآن قاعدة البيانات (البيئة كبديل).
- **صفحة الدخول**: خلفية بملء الشاشة + مربع دخول قابل للسحب بالماوس (يُحفظ موضعه محلياً).
