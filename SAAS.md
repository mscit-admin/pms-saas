...
## لوحة المشرف الأعلى (Super-admin) — `/control`

لوحة بقائمة جانبية (ثنائية اللغة + فاتح/داكن)، أقسامها محكومة بصلاحيات المشرف:

- **لوحة المعلومات** (`view_dashboard`): إحصاءات — عدد العملاء، مفعّل/معلّق،
  مجموع المستخدمين/المشاريع، توزيع الخطط.
- **العملاء** (`manage_tenants`): إضافة/تعليق/حذف، خطة/حصص/وحدات، إعادة كلمة مرور
  الأدمن، و**إدارة مركزية** لكل عميل (مستخدمون + أدوار داخل قاعدته عبر `runInTenant`).
- **الإعدادات**: **الهوية البصرية** للمنصّة (`manage_branding`: اسم/لون/شعار في
  `control_settings`)، و**المشرفون + الصلاحيات** (`manage_admins`: حسابات
  المشرفين وصلاحياتهم — كتالوج `CONTROL_PERMISSIONS`).

الحساب المبذور من البيئة «مالك» بكل الصلاحيات. القديم أدناه (مرجع):

شاشة لإدارة المستأجرين بحساب مشرف منفصل (في قاعدة التحكّم، لا في أي مستأجر).

- **الدخول:** `http://<الخادم>/control` (أو `:4447/control/login`). الحساب من
  `CONTROL_ADMIN_USER` / `CONTROL_ADMIN_PASSWORD` (يُبذَر عند الإقلاع).
- **القدرات:** إضافة مستأجر (اسم + slug + كلمة مرور أدمن)، تعليق/تفعيل، حذف
  (يُسقط قاعدته)، ضبط **الخطة والحصص** (أقصى مستخدمين/مشاريع) و**الوحدات**
  (analytics · dependencies · ai · multiJira · alerts)، وإعادة تعيين كلمة مرور
  أدمن المستأجر.
- **الفرض:** حدّ المستخدمين يُفرَض عند إنشاء مستخدم، وحدّ المشاريع عند إنشاء مشروع
  (`assertUserQuota`/`assertProjectQuota`). الوحدات مخزّنة وتُعرض، وتُفرَض عبر
  `requireFeature` حيث تُربط.
- **المعمارية:** مسارات `/api/control/*` (مستثناة من حارس المستأجر، محميّة
  بـ `requireSuperAdmin`)؛ جلسة بكوكي `pms_control_session` وسرّ مستقلّ؛ جدولا
  التحكّم `control_admins` و أعمدة `organizations` الإضافية (حصص/وحدات).
